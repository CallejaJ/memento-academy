...
DROP VIEW IF EXISTS public.game_leaderboard_modes;

CREATE OR REPLACE VIEW public.game_leaderboard_modes WITH (security_invoker = true) AS
SELECT 
  gs.user_id,
...
LEFT JOIN public.user_wallets uw ON gs.user_id = uw.user_id
WHERE gs.finished_at IS NOT NULL
GROUP BY gs.user_id, p.full_name, p.avatar_url, uw.wallet_address, gs.game_mode;

GRANT SELECT ON public.game_leaderboard_modes TO anon, authenticated, service_role;
