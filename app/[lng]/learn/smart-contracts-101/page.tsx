  ...
  en: {
    ...
    sections: {
      1: {
        ...
        cases: {
          fin: { title: "💰 Financial Use Cases", desc: "DeFi protocols, lending/borrowing, DEXs, stablecoins, derivatives", ex: "Example: Uniswap automatically swaps tokens based on AMM formula" },
          nft: { title: "🎮 NFT & Gaming", desc: "NFT minting, marketplaces, in-game economies, ownership", ex: "Example: CryptoPunks minting contract created 10,000 unique avatars" },
          gov: { title: "🏛️ Governance", desc: "DAOs, voting mechanisms, proposals, treasury management", ex: "Example: MakerDAO governance controls $5B+ protocol" },
          supply: { title: "🚛 Supply Chain", desc: "Tracking, authenticity verification, automated payments", ex: "Example: Walmart uses blockchain to track food supply" }
        },
        hello: {
      ...
      2: {
        ...
        types: {
          ...
          val: { title: "Value Types", list: ["<code class=\"text-cyan-300\">bool</code>: true/false", "<code class=\"text-cyan-300\">uint/int</code>: unsigned/signed integers", "<code class=\"text-cyan-300\">address</code>: Ethereum address", "<code class=\"text-cyan-300\">bytes</code>: byte arrays"] },
          ref: { title: "Reference Types", list: ["<code class=\"text-cyan-300\">arrays</code>: fixed or dynamic", "<code class=\"text-cyan-300\">struct</code>: custom data structures", "<code class=\"text-cyan-300\">mapping</code>: key-value storage", "<code class=\"text-cyan-300\">string</code>: UTF-8 encoded text"] }
        },
        funcs: {
      ...
      3: {
        ...
        tools: {
          ...
          hardhat: { title: "Hardhat", desc: "Professional development environment", list: ["Built-in testing framework", "TypeScript support", "Extensive plugin ecosystem"] },
          remix: { title: "Remix IDE", desc: "Browser-based IDE for beginners", list: ["No setup required", "Visual debugger", "Deploy to testnets easily"] }
        },
        process: {
          title: "Deployment Process",
          steps: [
            { title: "Compile:", desc: "Convert Solidity to bytecode" },
            { title: "Test Locally:", desc: "Run on Hardhat Network (local blockchain)" },
            { title: "Deploy to Testnet:", desc: "Sepolia, Goerli - free test ETH" },
            { title: "Verify Contract:", desc: "Publish source code on Etherscan" },
      ...
      4: {
        ...
        vulns: {
          ...
          reent: { title: "Reentrancy", desc: "Attacker calls back into contract before state updates", fix: "✅ Fix: Use Checks-Effects-Interactions pattern or ReentrancyGuard" },
          overflow: { title: "Integer Overflow/Underflow", desc: "Arithmetic operations exceed max/min values", fix: "✅ Fix: Use Solidity 0.8.0+ (built-in checks) or SafeMath library" },
          access: { title: "Access Control", desc: "Unauthorized users can call privileged functions", fix: "✅ Fix: Use modifiers (onlyOwner) and OpenZeppelin's AccessControl" }
        },
        list: {
      ...
      5: {
        ...
        frameworks: {
          ...
          hardhat: { title: "Hardhat Testing", desc: "JavaScript/TypeScript with Mocha & Chai" },
          foundry: { title: "Foundry Testing", desc: "Write tests in Solidity itself" }
        },
        areas: {
          title: "Test Coverage Areas",
          list: [
            { title: "Unit Tests:", desc: "Test individual functions in isolation" },
            { title: "Integration Tests:", desc: "Test interactions between contracts" },
            { title: "Edge Cases:", desc: "Zero values, max values, empty arrays" },
      ...
      6: {
        ...
        patterns: {
          proxy: { title: "🔄 Proxy Pattern", desc: "Enable upgradeable contracts via delegation", ex: "Used by: USDC, USDT, Compound" },
          factory: { title: "🏭 Factory Pattern", desc: "Deploy multiple instances programmatically", ex: "Used by: Uniswap (pair creation)" },
          diamond: { title: "💎 Diamond Pattern", desc: "Modular contracts with multiple facets", ex: "Breaks 24KB contract size limit" },
          pull: { title: "🗳️ Pull Payment", desc: "Let users withdraw instead of push payments", ex: "Safer than direct transfers" }
        },
        gas: {
  ...
  es: {
    ...
    sections: {
      1: {
        ...
        cases: {
          fin: { title: "💰 Casos Financieros", desc: "Protocolos DeFi, préstamos, DEXs, stablecoins", ex: "Ejemplo: Uniswap intercambia tokens automáticamente" },
          nft: { title: "🎮 NFT y Gaming", desc: "Minteo de NFTs, marketplaces, economías in-game", ex: "Ejemplo: CryptoPunks creó 10,000 avatares únicos" },
          gov: { title: "🏛️ Gobernanza", desc: "DAOs, votaciones, propuestas, tesorería", ex: "Ejemplo: Gobernanza de MakerDAO controla protocolo" },
          supply: { title: "🚛 Cadena de Suministro", desc: "Rastreo, verificación de autenticidad, pagos", ex: "Ejemplo: Walmart usa blockchain para rastrear alimentos" }
        },
        hello: {
      ...
      2: {
        ...
        types: {
          ...
          val: { title: "Tipos de Valor", list: ["<code class=\"text-cyan-300\">bool</code>: verdadero/falso", "<code class=\"text-cyan-300\">uint/int</code>: enteros sin/con signo", "<code class=\"text-cyan-300\">address</code>: dirección Ethereum", "<code class=\"text-cyan-300\">bytes</code>: arrays de bytes"] },
          ref: { title: "Tipos de Referencia", list: ["<code class=\"text-cyan-300\">arrays</code>: fijos o dinámicos", "<code class=\"text-cyan-300\">struct</code>: estructuras personalizadas", "<code class=\"text-cyan-300\">mapping</code>: almacenamiento clave-valor", "<code class=\"text-cyan-300\">string</code>: texto codificado UTF-8"] }
        },
        funcs: {
      ...
      3: {
        ...
        tools: {
          ...
          hardhat: { title: "Hardhat", desc: "Entorno de desarrollo profesional", list: ["Framework de testing incluido", "Soporte TypeScript", "Ecosistema extenso de plugins"] },
          remix: { title: "Remix IDE", desc: "IDE en navegador para principiantes", list: ["Sin instalación", "Depurador visual", "Despliegue fácil a testnets"] }
        },
        process: {
          title: "Proceso de Despliegue",
          steps: [
            { title: "Compilar:", desc: "Convertir Solidity a bytecode" },
            { title: "Test Local:", desc: "Ejecutar en red Hardhat (blockchain local)" },
            { title: "Desplegar a Testnet:", desc: "Sepolia, Goerli - ETH de prueba gratis" },
            { title: "Verificar Contrato:", desc: "Publicar código fuente en Etherscan" },
      ...
      4: {
        ...
        vulns: {
          ...
          reent: { title: "Reentrancia", desc: "Atacante llama de vuelta al contrato antes de actualizar estado", fix: "✅ Fix: Patrón Checks-Effects-Interactions o ReentrancyGuard" },
          overflow: { title: "Overflow/Underflow Enteros", desc: "Operaciones exceden valores máx/min", fix: "✅ Fix: Usar Solidity 0.8.0+ (chequeos nativos)" },
          access: { title: "Control de Acceso", desc: "Usuarios no autorizados llaman funciones privilegiadas", fix: "✅ Fix: Usar modifiers (onlyOwner) y AccessControl de OpenZeppelin" }
        },
        list: {
      ...
      5: {
        ...
        frameworks: {
          ...
          hardhat: { title: "Hardhat Testing", desc: "JavaScript/TypeScript con Mocha & Chai" },
          foundry: { title: "Foundry Testing", desc: "Escribe tests en Solidity mismo" }
        },
        areas: {
          title: "Áreas de Cobertura de Test",
          list: [
            { title: "Tests Unitarios:", desc: "Probar funciones individuales aisladas" },
            { title: "Tests de Integración:", desc: "Probar interacciones entre contratos" },
            { title: "Casos Borde:", desc: "Valores cero, máximos, arrays vacíos" },
            { title: "Control de Acceso:", desc: "Intentos de acceso no autorizado" },
      ...
      6: {
        ...
        patterns: {
          proxy: { title: "🔄 Patrón Proxy", desc: "Permite contratos actualizables vía delegación", ex: "Usado por: USDC, USDT" },
          factory: { title: "🏭 Patrón Factory", desc: "Desplegar múltiples instancias programáticamente", ex: "Usado por: Uniswap (creación de pares)" },
          diamond: { title: "💎 Patrón Diamond", desc: "Contratos modulares con múltiples facetas", ex: "Rompe el límite de tamaño de 24KB" },
          pull: { title: "🗳️ Pull Payment", desc: "Permitir retirar en lugar de enviar pagos", ex: "Más seguro que transferencias directas" }
        },
        gas: {
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto text-center space-y-6">
            ...
            <div className="flex flex-wrap gap-4 justify-center pt-4">
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-lg px-6 py-3">
                <div className="text-2xl font-bold text-cyan-400">{t.level_val}</div>
                    ...
                    <div className="px-6 pb-6">
                      <div className="space-y-4 text-slate-300">
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5">
                          <h4 className="text-blue-400 font-semibold mb-3">{t.sections[1].chars.title}</h4>
                          <ul className="space-y-2 text-sm">
                            {t.sections[1].chars.list.map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: `• ${item}` }} />)}
                          </ul>
                        </div>

                        <div className="grid md:grid-cols-2 gap-4">
                          <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                            <h5 className="text-green-400 font-semibold mb-2">{t.sections[1].cases.fin.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[1].cases.fin.desc}</p>
                            <p className="text-xs italic text-slate-500">{t.sections[1].cases.fin.ex}</p>
                          </div>
                          <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
                            <h5 className="text-purple-400 font-semibold mb-2">{t.sections[1].cases.nft.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[1].cases.nft.desc}</p>
                            <p className="text-xs italic text-slate-500">{t.sections[1].cases.nft.ex}</p>
                          </div>
                          <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-4">
                            <h5 className="text-cyan-400 font-semibold mb-2">{t.sections[1].cases.gov.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[1].cases.gov.desc}</p>
                            <p className="text-xs italic text-slate-500">{t.sections[1].cases.gov.ex}</p>
                          </div>
                          <div className="bg-pink-500/10 border border-pink-500/20 rounded-lg p-4">
                            <h5 className="text-pink-400 font-semibold mb-2">{t.sections[1].cases.supply.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[1].cases.supply.desc}</p>
                            <p className="text-xs italic text-slate-500">{t.sections[1].cases.supply.ex}</p>
                          </div>
                        </div>

                        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                          <h5 className="text-white font-semibold mb-2">{t.sections[1].hello.title}</h5>
                          <p className="text-sm mb-3">{t.sections[1].hello.desc}</p>
                        ...
                        <div className="mt-6 flex justify-center">
                          <SectionCompleteButton courseId="smart-contracts-101" sectionId="section-1" sectionNumber={1} totalSections={6} lng={lng} />
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="px-6 pb-6">
                      <div className="space-y-4 text-slate-300">
                        ...
                        <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-5">
                          <h4 className="text-cyan-400 font-semibold mb-3">{t.sections[2].types.title}</h4>
                          <div className="grid md:grid-cols-2 gap-3 text-sm">
                            <div>
                              <p className="text-white font-semibold mb-2">{t.sections[2].types.val.title}</p>
                              <ul className="text-xs text-slate-400 space-y-1">
                                {t.sections[2].types.val.list.map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: `• ${item}` }} />)}
                              </ul>
                            </div>
                            <div>
                              <p className="text-white font-semibold mb-2">{t.sections[2].types.ref.title}</p>
                              <ul className="text-xs text-slate-400 space-y-1">
                                {t.sections[2].types.ref.list.map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: `• ${item}` }} />)}
                              </ul>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                          <h5 className="text-white font-semibold mb-2">{t.sections[2].funcs.title}</h5>
                          ...
                          <p className="text-sm" dangerouslySetInnerHTML={{ __html: t.sections[2].funcs.desc }} />
                        </div>

                        <div className="mt-6 flex justify-center">
                          <SectionCompleteButton courseId="smart-contracts-101" sectionId="section-2" sectionNumber={2} totalSections={6} lng={lng} />
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="px-6 pb-6">
                      <div className="space-y-4 text-slate-300">
                        ...
                        <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-5">
                          <h4 className="text-green-400 font-semibold mb-3">{t.sections[3].tools.title}</h4>
                          <div className="grid md:grid-cols-2 gap-4">
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-2">{t.sections[3].tools.hardhat.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[3].tools.hardhat.desc}</p>
                              <ul className="text-xs text-slate-500 space-y-0.5">
                                {t.sections[3].tools.hardhat.list.map((item, i) => <li key={i}>• {item}</li>)}
                              </ul>
                            </div>
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-2">{t.sections[3].tools.remix.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[3].tools.remix.desc}</p>
                              <ul className="text-xs text-slate-500 space-y-0.5">
                                {t.sections[3].tools.remix.list.map((item, i) => <li key={i}>• {item}</li>)}
                              </ul>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                          <h5 className="text-blue-400 font-semibold mb-2">{t.sections[3].process.title}</h5>
                        ...
                        <div className="mt-6 flex justify-center">
                          <SectionCompleteButton courseId="smart-contracts-101" sectionId="section-3" sectionNumber={3} totalSections={6} lng={lng} />
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="px-6 pb-6">
                      <div className="space-y-4 text-slate-300">
                        ...
                        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-5">
                         <h4 className="text-red-400 font-semibold mb-3 flex items-center gap-2">{t.sections[4].vulns.title}</h4>
                          <div className="space-y-3">
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-1">{t.sections[4].vulns.reent.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[4].vulns.reent.desc}</p>
                              <p className="text-xs text-green-400">{t.sections[4].vulns.reent.fix}</p>
                            </div>
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-1">{t.sections[4].vulns.overflow.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[4].vulns.overflow.desc}</p>
                              <p className="text-xs text-green-400">{t.sections[4].vulns.overflow.fix}</p>
                            </div>
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-1">{t.sections[4].vulns.access.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[4].vulns.access.desc}</p>
                              <p className="text-xs text-green-400">{t.sections[4].vulns.access.fix}</p>
                            </div>
                          </div>
                        </div>

                        <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-4">
                          <h5 className="text-cyan-400 font-semibold mb-2">{t.sections[4].list.title}</h5>
                          <ul className="text-sm space-y-1">
                            {t.sections[4].list.items.map((item, i) => <li key={i}>{item}</li>)}
                          </ul>
                        </div>

                        <div className="mt-6 flex justify-center">
                          <SectionCompleteButton courseId="smart-contracts-101" sectionId="section-4" sectionNumber={4} totalSections={6} lng={lng} />
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="px-6 pb-6">
                      <div className="space-y-4 text-slate-300">
                        ...
                        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-5">
                          <h4 className="text-yellow-400 font-semibold mb-3">{t.sections[5].frameworks.title}</h4>
                          <div className="grid md:grid-cols-2 gap-3">
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-2">{t.sections[5].frameworks.hardhat.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[5].frameworks.hardhat.desc}</p>
                            ...
                            <div className="bg-slate-800/50 rounded p-3">
                              <h5 className="text-white font-semibold text-sm mb-2">{t.sections[5].frameworks.foundry.title}</h5>
                              <p className="text-xs text-slate-400 mb-2">{t.sections[5].frameworks.foundry.desc}</p>
                              <div className="bg-slate-900 rounded p-2 font-mono text-xs">
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                          <h5 className="text-blue-400 font-semibold mb-2">{t.sections[5].areas.title}</h5>
                          <ul className="space-y-2 text-sm">
                        ...
                        <div className="mt-6 flex justify-center">
                          <SectionCompleteButton courseId="smart-contracts-101" sectionId="section-5" sectionNumber={5} totalSections={6} lng={lng} />
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {expandedSections.includes(5) && (
                    <div className="px-6 pb-6">
                      <div className="space-y-4 text-slate-300">
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                          <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
                            <h5 className="text-purple-400 font-semibold mb-2">{t.sections[6].patterns.proxy.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[6].patterns.proxy.desc}</p>
                            <p className="text-xs text-slate-500">{t.sections[6].patterns.proxy.ex}</p>
                          </div>
                          <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                            <h5 className="text-blue-400 font-semibold mb-2">{t.sections[6].patterns.factory.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[6].patterns.factory.desc}</p>
                            <p className="text-xs text-slate-500">{t.sections[6].patterns.factory.ex}</p>
                          </div>
                          <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                            <h5 className="text-green-400 font-semibold mb-2">{t.sections[6].patterns.diamond.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[6].patterns.diamond.desc}</p>
                            <p className="text-xs text-slate-500">{t.sections[6].patterns.diamond.ex}</p>
                          </div>
                          <div className="bg-pink-500/10 border border-pink-500/20 rounded-lg p-4">
                            <h5 className="text-pink-400 font-semibold mb-2">{t.sections[6].patterns.pull.title}</h5>
                            <p className="text-xs text-slate-400 mb-2">{t.sections[6].patterns.pull.desc}</p>
                            <p className="text-xs text-slate-500">{t.sections[6].patterns.pull.ex}</p>
                          </div>
                        </div>

                        <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-5">
                          <h4 className="text-cyan-400 font-semibold mb-3">{t.sections[6].gas.title}</h4>
                          <ul className="space-y-1 text-sm">
                            {t.sections[6].gas.list.map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: item }} />)}
                          </ul>
                        </div>

                        <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-lg p-5 text-center">
                          <h5 className="text-white font-semibold mb-2">{t.sections[6].ready.title}</h5>
                          <p className="text-sm text-slate-400">{t.sections[6].ready.desc}</p>
                        </div>

                        <div className="mt-6 flex justify-center">
                          <SectionCompleteButton courseId="smart-contracts-101" sectionId="section-6" sectionNumber={6} totalSections={6} lng={lng} />
                        </div>
                      </div>
                    </div>
                  )}
